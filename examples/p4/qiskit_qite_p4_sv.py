...
def get_expectation_value(circuit: QuantumCircuit, H:SparsePauliOp):
    ...
    # plt.savefig(f"cx-rx-ry-rz-x-basis_sets.png", dpi=50)

    return expectation_value

# Created for some test purposes
...
def QITE_step(circuit, p4_sparse, ea_prev):
    circuit_temp = circuit.copy()
    ea = ea_prev
    for ib in range (30):   #for ib in range (nsteps):
        ...
        circuit_temp.append(real_evolution_gate, [0,1,2,3,4,5])
        ea = get_expectation_value(circuit_temp, p4_sparse)
        if abs(ea) > abs(ea_prev):
            ea_prev = ea
            circuit = circuit_temp.copy()
            print(f"Expectation value: {ea}")
        else:
            # Comment two lines below and uncomment the two after those 
            #     if you want to discard gates that don't give better expectation during each time evolution step.
            circuit = circuit_temp.copy()
            print(f"Expectation value: {ea}")
            # circuit_temp = circuit.copy()
            # print(f"Evolution not appended.")
...
print(f"Initial expectation value: {ea}")

QITE_step(circuit, p4_sparse, ea)
